...
function freakify(text: string) {
  ...
  return text.split('').map((c) => (c === ' ' ? c : c + up.slice(0, Math.random()*3|0).join('') + mid.slice(0, Math.random()*2|0).join('') + down.slice(0, Math.random()*3|0).join(''))).join('');
}
const command: PrefixCommand = {
  name: 'freaky',
  description: 'Freakify text (zalgo)',
  usage: '?freaky <text>',
  category: 'fun',
  async execute(message, args) {
    const text = args.join(' ');
    if (!text) return void message.reply('Provide some text.');
    await message.reply(freakify(text));
  },
};
